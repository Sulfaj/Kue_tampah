...
// Skema validasi form—ubah aturan atau tambah field sesuai kebutuhan checkout.
const formSchema = z
  .object({
...
// Nilai default form—tentukan pilihan awal agar sesuai alur pesananmu.
...
export const CheckoutForm = () => {
  ...
  const whatsappNumber = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER ?? '081523961519';
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      // Bangun pesan WhatsApp—sesuaikan template di lib/whatsapp.ts bila format ingin diubah.
      ...
      if (data.paymentMethod === 'qris') {
        // Ubah generator QRIS di lib/qris.ts jika memakai gateway pembayaran lain.
  ...
  return (
    <>
      {/* Form checkout utama—atur susunan field, copy, dan styling Tailwind di bawah ini. */}
      <form
      ...
      {/* Modal QRIS—atur tampilan di components/QRISModal.tsx bila ingin branding khusus. */}
      <QRISModal
        open={qrisData.open}
        onOpenChange={(open) => {
